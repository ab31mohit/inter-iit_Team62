...
class DataPlotterNode(Node):
    def __init__(self,odometry_file_path, plot_file_path):
...
def main(args=None):
    ...
    try:
        csv_directory_path = os.path.join(os.path.expanduser("~"), "ros2/inter-iit_ws/src/Inter-IIT_IdeaForge-PS/detection_tests/detection_logs/csv")
        plots_directory_path = os.path.join(os.path.expanduser("~"), "ros2/inter-iit_ws/src/Inter-IIT_IdeaForge-PS/detection_tests/detection_logs/plots")

        odometry_files = sorted([f for f in os.listdir(csv_directory_path) if f.startswith("odometry_data_") and f.endswith(".csv")])

        for odometry_file in odometry_files:
            ...
            data_plotter = DataPlotterNode(odometry_file_path, plot_file_path)
